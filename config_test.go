...
func TestLoadConfig(t *testing.T) {
	...
	if cfg.RabbitMQURL != "amqp://localhost:5672" {
		t.Errorf("rabbitmq_url: got %q, want %q", cfg.RabbitMQURL, "amqp://localhost:5672")
	}
...
func TestLoadConfigMissingRabbitMQURL(t *testing.T) {
	dir := t.TempDir()
...
func TestLoadConfigUnknownField(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "config.jsonnet")
	content := `{ rabbitmq_url: "amqp://localhost:5672", unknown_field: "value" }`
